...
export const addons: PresetProperty<'addons', StorybookConfig> = [
  dirname(require.resolve(join('@storybook/preset-server-webpack', 'package.json'))),
  dirname(require.resolve(join('@storybook/server', 'package.json'))),
];
...
export const webpack: StorybookConfig['webpack'] = async (config) => {
  ...
  config.resolve.alias = {
    ...
    '@storybook/server': dirname(require.resolve(join('@storybook/server', 'package.json'))),
